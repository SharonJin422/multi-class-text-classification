...
import time
import logging
from process2 import load_dataset
import numpy as np
import pdb
...
def train_cnn():
	...
	x_raw, y_raw = load_dataset("./data/train_data.p")
	# parameter_file = sys.argv[2]
	# params = json.loads(open(parameter_file).read())

	"""Step 1: pad each sentence to the same length and map each word to an id"""
	max_length = max([len(x) for x in x_raw])
	max_length = 8000
	logging.info('The maximum length of all sentences: {}'.format(max_length))
	pdb.set_trace()
